...
您应该能看到类似这样的输出：
```
inet <IP地址1>/24 scope global eth0
inet <IP地址2>/24 scope global eth0
inet <IP地址3>/24 scope global eth0
inet <IP地址4>/24 scope global eth0
```

### 步骤2：验证IP连通性

```bash
# 测试每个IP是否可达（将 <IP地址> 替换为实际的IP地址）
ping -c 3 <IP地址1>
ping -c 3 <IP地址2>
ping -c 3 <IP地址3>
ping -c 3 <IP地址4>
